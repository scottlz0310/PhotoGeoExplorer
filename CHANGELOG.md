...
- `MapPaneViewModel` の重複ズーム正規化解消とマーカー表示ロジック分離（#150 Phase 4）(#198, #207)
  ...
  - thread-owl / Copilot レビュー指摘対応（blocking）: `UpdateMarkersFromSelectionAsync` で `_map`/`_markerLayer` を `await` 前にローカル変数へ退避していたため、`await`（キャンセル完了待ち・メタデータ読み込み待ち）中に `OnCleanup()` が走ると破棄済みの map/layer を後続の `MapMarkerPresenter` 呼び出しが更新し得る回帰があった。各 `await` 後に現在のフィールドを再取得してから使うよう修正し、`UpdateMarkersFromSelectionAsyncDoesNotThrowWhenCleanupHappensDuringLoad`（新規）でロード待機中の `Cleanup()` 発生を固定するテストを追加
